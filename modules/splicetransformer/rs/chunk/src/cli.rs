...
pub const DEFAULT_FLANK_SIZE: u32 = 4_000;
pub const DEFAULT_MIN_CONTIG_SIZE: u32 = 500;

#[derive(Debug, Parser)]
#[command(
    name = "splicetransformer-chunk",
    about = "Chunk a genome FASTA/2bit into SpliceTransformer-ready FASTA shards",
...
mod tests {
    ...
    fn cli_defaults_gzip_to_false_and_outdir_to_current_directory() {
        let args = Args::parse_from(["splicetransformer-chunk"]);
    ...
    fn cli_enables_gzip_flag_when_requested() {
        let args = Args::parse_from(["splicetransformer-chunk", "--gzip"]);
